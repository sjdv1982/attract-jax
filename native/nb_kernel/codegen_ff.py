...
__all__ = ["lj_energy", "elec_energy", "load_params", "FFParams"]
'''
...
def _grad_wrapper(name: str, rot: str, rot_policy: str, cap_name: str) -> str:
    """Generate the extern "C" energy+gradient wrapper."""
    sym = f"nb_kernel_{rot}_grad"
    return f"""\
/**
 * {sym} — energy + gradient, {rot} rotation, {name} force field.
 */
extern "C" int {sym}(
...
{{
  ...
  run_pose_loop_fused<{rot_policy}, {cap_name}, true>(
...
def _energy_wrapper(name: str, rot: str, rot_policy: str, cap_name: str) -> str:
    """Generate the extern "C" energy-only wrapper."""
    sym = f"nb_kernel_{rot}_energy"
    return f"""\
/**
 * {sym} — energy only, {rot} rotation, {name} force field.
...
{{
  ...
  const int32_t nthreads = cfg->num_threads > 0 ? cfg->num_threads : 1;
  omp_set_num_threads(nthreads);
  run_pose_loop_fused<{rot_policy}, {cap_name}, false>(
...
"""
    ...
    parts.append(f"// Naming convention: nb_kernel_<rot>_<grad|energy>")
    parts.append(
        "// ---------------------------------------------------------------------------"
    )
    parts.append("")
    ...
    for rot in rotations:
        ...
        if has_grad:
            parts.append(_grad_wrapper(name, rot, rot_policy, cap_name))
        parts.append(_energy_wrapper(name, rot, rot_policy, cap_name))
    ...
    if has_grad:
        print(f"  Wrappers   : nb_kernel_euler_grad, nb_kernel_euler_energy")
    else:
        print(f"  Wrappers   : nb_kernel_euler_energy  (no gradient headers found)")
