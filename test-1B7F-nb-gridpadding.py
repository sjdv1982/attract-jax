...
@jit
def nb_energy_single(nb_index, offset, ligand_struc, ligand_atom, all_coors_lig, rec_atomtypes, lig_atomtypes, neighbours):

    receptor_atom = neighbours[nb_index + offset - 1]
...
nb_energy = jnp.vectorize(nb_energy_single, excluded=(1,4,5,6,7))
...
def main(coor_rec, rec_atomtypes, coor_lig, lig_atomtypes, ff, grid):
    ...
    for i, chs in enumerate(chunks[::-1]):
        for ch in chs:
            ...
            offset = i
            energy = nb_energy(chunk_nb_index, offset, chunk_ligand_struc, chunk_ligand_atom, all_coors_lig, rec_atomtypes, lig_atomtypes, grid.neighbours)
...
lp = list(range(10)[::-1])
padding = [0] * 5 + lp + lp + lp
padding  += list(range(max_contacts-len(padding))[::-1])
grid2 = pad_grid(grid, padding=padding)

energy = main(coor_rec, rec_atomtypes, coor_lig, lig_atomtypes, ff, grid2)
print(energy)

"""
# Debug padding
nonempty=(grid.neighbour_grid[:, :, :, 0] > 0) 
nonempty2=(grid2.neighbour_grid[:, :, :, 0] > 0) 
assert np.all(np.equal(nonempty, nonempty2))
assert np.all(np.equal(grid.neighbour_grid[:, :, :, 0], grid2.neighbour_grid[:, :, :, 0]))

nonempty_ind = list(zip(*np.where(nonempty)))
for n in range(len(nonempty_ind)):
    if n % 1000 == 0:
        print(n, len(nonempty_ind))
    i = nonempty_ind[n]
    # assert grid.neighbour_grid[i][0] == grid2.neighbour_grid[i][0], i  # done above
    for ii in range(grid.neighbour_grid[i][0]):
        v1 = grid.neighbours[grid.neighbour_grid[i][1]+ii-1]
        v2 = grid2.neighbours[grid2.neighbour_grid[i][1]+ii-1]
        assert v1 == v2, (v1, v2, n, i, ii, grid.neighbour_grid[i], grid2.neighbour_grid[i])
"""        