# Retiling horizontally (see "status of ATTRACT-JAX project")
...
if NSTRUC < len(mat):
    print(f"Select top {NSTRUC} structures", file=sys.stderr)
    mat = mat[:NSTRUC]
...
lp5 = list(range(5)[::-1])
lp = list(range(10)[::-1])
padding = [0] * 5 + lp5 + lp5 + lp5 + lp + lp + lp
...
@jit
def potential_energy(mat, coor_lig, lig_atomtype_pos, grid):
...
def neighbour_energy(mat, coor_rec, rec_atomtypes, coor_lig, lig_atomtypes, ff, grid, chunks, grid_dim):
    coor_lig2 = jnp.concatenate((coor_lig, jnp.ones((len(coor_lig),1))),axis=1)
    all_coors_lig = jnp.einsum("jk,ikl->ijl", coor_lig2, mat) #diagonally broadcasted form of coor_lig.dot(mat)
    all_coors_lig = all_coors_lig[:, :, :3]
...
def get_contact_lengths(mat, coor_lig,  grid):
    coor_lig2 = jnp.concatenate((coor_lig, jnp.ones((len(coor_lig),1))),axis=1)
    all_coors_lig = jnp.einsum("jk,ikl->ijl", coor_lig2, mat) #diagonally broadcasted form of coor_lig.dot(mat)
    all_coors_lig = all_coors_lig[:, :, :3]
...
@partial(jit, static_argnames=("grid_dim","chunks"))
def main(mat, coor_rec, rec_atomtypes, coor_lig, lig_atomtypes, lig_atomtype_pos, ff, grid, chunks, grid_dim):

    nb_energies = neighbour_energy(mat, coor_rec, rec_atomtypes, coor_lig, lig_atomtypes, ff, grid, chunks, grid_dim)
    pot_energies = potential_energy(mat, coor_lig, lig_atomtype_pos, grid)
...
contact_lengths = get_contact_lengths(mat, coor_lig, grid)
...
for n in (0,1,2,3,4,5,10,15,20,30,40,50)[::-1]:
...
if CALC_GRADS:
    ...
    (_, energies), gradients = vgrad_main(mat, coor_rec, rec_atomtypes, coor_lig, lig_atomtypes, lig_atomtype_pos, ff, grid, chunks, grid_dim)
    ...
    _, energies = main(mat, coor_rec, rec_atomtypes, coor_lig, lig_atomtypes, lig_atomtype_pos, ff, grid, chunks, grid_dim)
...
main(mat, coor_rec, rec_atomtypes, coor_lig, lig_atomtypes, lig_atomtype_pos, ff, grid, chunks, grid_dim)
